...
export default function Home() {
  ...
  return (
    <div style={{ maxWidth: '480px', margin: '0 auto', padding: '16px', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif', backgroundColor: '#f9f9f9', minHeight: '100vh' }}>
      ...
      {/* BLOQUE DE RESULTADOS SOLICITADO */}
      {data && !data.necesitaAclaracion && (
        <div>
          {/* Fila Bloque Superior: En paralelo, ambas en BLANCO */}
          <div style={{ display: 'flex', gap: '12px', marginBottom: '12px' }}>
            
            {/* Tarjeta 1: Precio Tasación */}
            <div style={{ flex: 1, backgroundColor: '#ffffff', color: '#111111', padding: '16px', borderRadius: '12px', border: '1px solid #e5e7eb', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
              <span style={{ fontSize: '11px', fontWeight: 'bold', color: '#dc2626', textTransform: 'uppercase', letterSpacing: '0.3px' }}>PRECIO TASACIÓN</span>
              <div style={{ margin: '12px 0 0 0' }}>
                <span style={{ fontSize: '26px', fontWeight: '800', color: '#111827' }}>{data.precioTasacion}€</span>
              </div>
            </div>

            {/* Tarjeta 2: Precio B2B Precio Compraventas */}
            <div style={{ flex: 1, backgroundColor: '#ffffff', color: '#111111', padding: '16px', borderRadius: '12px', border: '1px solid #e5e7eb', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
              <span style={{ fontSize: '11px', fontWeight: 'bold', color: '#1e3a8a', textTransform: 'uppercase', letterSpacing: '0.3px' }}>PRECIO B2B COMPRAVENTAS</span>
              <div style={{ margin: '12px 0 0 0' }}>
          ...
          {/* Fila Inferior: Las 3 etiquetas horizontales solicitadas */}
          <div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
            ...
            <div style={{ flex: 1.1, backgroundColor: '#fff', padding: '12px 10px', borderRadius: '12px', border: '1px solid #eee' }}>
              <span style={{ fontSize: '10px', color: '#e11d48', display: 'block', fontWeight: 'bold', textTransform: 'uppercase', marginBottom: '2px' }}>MÁS BARATO INTERNET</span>
              <span style={{ fontSize: '15px', fontWeight: 'bold', color: '#333', display: 'block' }}>{data.masBaratoInternet} €</span>
            </div>

            {/* 2. Precio venta medio unidades similares */}
            <div style={{ flex: 1.2, backgroundColor: '#fff', padding: '12px 10px', borderRadius: '12px', border: '1px solid #eee' }}>
              <span style={{ fontSize: '10px', color: '#4b5563', display: 'block', fontWeight: 'bold', textTransform: 'uppercase', marginBottom: '2px' }}>PVP MEDIO SIMILARES</span>
              <span style={{ fontSize: '15px', fontWeight: 'bold', color: '#333', display: 'block' }}>{data.precioVentaMedio} €</span>
            </div>

            {/* 3. Tarjeta Rotación Coloreada */}
            <div style={{ 
              flex: 0.9, 
              ...
              <span style={{ fontSize: '16px', fontWeight: '900', color: estilosRotacion.color, marginTop: '2px' }}>{data.rotacion}</span>
            </div>

          </div>

          {/* Resumen Justificativo de Mecánica */}
          ...
          {/* Botón Realizar otra tasación en formato original azul */}
          <button
